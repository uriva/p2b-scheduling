getCalendlyEventTypes = (calendlyToken: string): { uri: string, name: string, slug: string, scheduling_url: string }[] => {
  ...
  userData = shouldParse1 ? jsonParse({ text: userRes.body }) : { resource: { uri: "" } }
  canFetchEvents = shouldParse1 ? true : false
  encodedUserUri = canFetchEvents ? urlEncode({ text: userData.resource.uri }) : { encoded: "" }
  ...
  eventsData = shouldParse2 ? jsonParse({ text: eventsRes.body }) : { collection: [] }
  eventTypes = eventsData.collection ? eventsData.collection : []
  return eventTypes
}
...
getCalendlyAvailableSlots = (calendlyToken: string, startTime: string, endTime: string): { name: string, slug: string, slots: { start_time: string, status: string }[] } => {
  ...
  slotsData = shouldParse3 ? jsonParse({ text: slotsRes.body }) : { collection: [] }
  slots = slotsData.collection ? slotsData.collection : []
